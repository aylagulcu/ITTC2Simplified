...
public class EventBasedR7 extends RobustnessEvaluatorBase {
	// The one that most resembles real measure. Also, this is the one which takes the longest to compute.
	// This is our another heuristic robustness measure. Minimization.
	// Soft Constraints are evaluated. 
	// Feasibility computation has been speeded up.
	// Both move and swap have been considered. 
	// An additional cost incurred for swap operations as in the real robustness measure.
...
//	List<HardConstraint> originalFeasConstraints;
	...
	public EventBasedR7(RobustnessManager manager) {
		super(manager);
...
//			if (tmp.size()>=3*hours)
	...
	private float evaluateEvent(Individual indiv, int ev) {
		...
		if (tOptValues.size()== 0)
			return this.avgEventPenalty*this.avgEventPenalty;
		float bestP= chooseBest(tOptValues);
		return bestP;
	}

	private float chooseBest(ArrayList<Float> tOptValues) {
		float min= tOptValues.get(0);
		
		for( int i=1; i< tOptValues.size(); i++){
			if (tOptValues.get(i) < min)
				min= tOptValues.get(i);
		} // end i for
		return min;
	}
