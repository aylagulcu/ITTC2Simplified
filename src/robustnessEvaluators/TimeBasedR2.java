...
public class TimeBasedR2 extends RobustnessEvaluatorBase {
	...
	public TimeBasedR2(RobustnessManager manager) {
		super(manager);
	...
	public void evaluateIndividual(Individual indiv) {
		...
		for (int time= 0; time< parameters.numTimeSlots; time++){
			...
			robustnessTime[time]= (float) ((Ctime[time] - Utime[time]) * Math.sqrt(Etime[time]) / Ctime[time]);
